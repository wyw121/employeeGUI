...
impl AdbService {
    ...
    pub async fn dump_ui_hierarchy(&self, device_id: &str) -> Result<String, Box<dyn std::error::Error>> {
        // 首先在设备上生成UI dump文件
        self.execute_adb_command(device_id, "uiautomator dump /sdcard/ui_hierarchy.xml").await?;
        
        // 然后拉取文件内容
        let content = self.execute_adb_command(device_id, "cat /sdcard/ui_hierarchy.xml").await?;
    ...
    pub async fn get_current_activity(&self, device_id: &str) -> Result<String, Box<dyn std::error::Error>> {
        self.execute_adb_command(device_id, "dumpsys activity activities | grep mCurrentFocus").await
    }

    /// 获取屏幕尺寸
    pub async fn get_screen_size(&self, device_id: &str) -> Result<String, Box<dyn std::error::Error>> {
        self.execute_adb_command(device_id, "wm size").await
    }
