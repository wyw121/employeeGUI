...
import type { DraggableItem, DroppableArea, DragResult } from '../drag-sort/types';
import type { ExtendedSmartScriptStep, LoopConfig } from '../loop-control/types';
...
export interface LoopDragIntegrationProps {
  ...
  /** 自定义步骤渲染函数 */
  renderStep?: (step: ExtendedSmartScriptStep, isDragging?: boolean) => React.ReactNode;
}
...
export const LoopDragIntegration: React.FC<LoopDragIntegrationProps> = ({
  ...
  function handleDragComplete(items: DraggableItem[], result: DragResult) {
    const updatedSteps = items
      ...
      .map(item => {
        ...
        return {
          ...step,
          parentLoopId: item.containerId === 'main' ? undefined : 
  ...
  // 拖拽验证
  function handleValidateDrag(item: DraggableItem, targetContainer: string): boolean {
    const step = item.data as ExtendedSmartScriptStep;
    
    // 循环开始/结束步骤不能拖入循环体
    if (targetContainer.startsWith('loop-') && 
  ...
  // 默认步骤渲染函数
  const defaultRenderStep = useCallback((step: ExtendedSmartScriptStep, isDragging = false) => {
    if (step.actionType === 'LOOP_START' || step.actionType === 'LOOP_END') {
      return (
        <LoopStepCard
          step={step}
          loopConfig={loopControl.getLoopConfig(step.id)}
          onConfigChange={(config) => loopControl.updateLoopConfig(step.id, config)}
    ...
    return (
      <Card
        ...
        style={{
          ...
          transform: isDragging ? 'rotate(5deg)' : undefined,
          cursor: isDragging ? 'grabbing' : 'grab',
          border: step.parentLoopId ? '2px solid #1890ff' : '1px solid #d9d9d9'
        }}
      >
  ...
  // 渲染项目函数
  const renderItem = useCallback((item: DraggableItem, isDragging = false) => {
    const step = item.data as ExtendedSmartScriptStep;
    return renderStep ? renderStep(step, isDragging) : defaultRenderStep(step, isDragging);
      ...
      <Space direction="vertical" size={16} style={{ width: '100%', marginBottom: 16 }}>
        <Card size="small" style={{ backgroundColor: '#f6ffed' }}>
          <Space>
            ...
            <span>拖拽步骤到循环体内可组织执行顺序，循环开始/结束步骤不能拖入循环体</span>
          </Space>
        </Card>
      </Space>
