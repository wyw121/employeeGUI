...
// 3. 定义测试样式（内联方式，不依赖CSS文件）
const testStyles = {
  card: {
    backgroundColor: '#ffffff !important',
    border: '2px solid #e879f9 !important',
    boxShadow: '0 4px 12px rgba(232, 121, 249, 0.15) !important',
    position: 'relative'
  },
  cardHead: {
    backgroundColor: '#fdf4ff !important',
    borderBottom: '2px solid #e879f9 !important'
  },
  cardHeadTitle: {
    ...
    boxShadow: '0 2px 4px rgba(168, 85, 247, 0.3) !important'
  },
  cardBody: {
    backgroundColor: '#ffffff !important',
  ...
  [...loopAnchors, ...inLoopSteps].forEach((element, index) => {
    ...
    if (cardHead) {
      ...
      if (titleFlex) {
        ...
        // 样式化头部所有文字和图标
        const textElements = titleFlex.querySelectorAll('.ant-typography, .anticon, .ant-tag, span');
...
function removeInlineTestStyles() {
  ...
  document.querySelectorAll('[data-test-style-applied]').forEach((element, index) => {
    ...
    // 移除测试标签
    const testLabel = element.querySelector('div');
    if (testLabel && testLabel.innerHTML === '🧪 TEST') {
      testLabel.remove();
    }
    ...
    allChildren.forEach(child => {
      if (child.dataset.originalStyle) {
        child.style.cssText = child.dataset.originalStyle;
      } else {
        child.style.cssText = '';
      }
    });
...
console.log('  testWhiteLoopInline.apply()    - 应用内联测试样式');
console.log('  testWhiteLoopInline.remove()   - 移除内联测试样式');
console.log('  testWhiteLoopInline.validate() - 验证样式效果');
console.log('  testWhiteLoopInline.toggle()   - 切换样式');

// 8. 自动检测并提示
if (loopAnchors.length > 0 || inLoopSteps.length > 0) {
  console.log('🚀 检测到循环元素，可以立即测试！');
  console.log('💡 执行 testWhiteLoopInline.apply() 立即查看效果');
